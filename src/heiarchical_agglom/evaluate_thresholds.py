...
def evaluate_thresholds(
    ...
    for crop in crops:
        ...
        print(
            f"best VOI for {crop_name} is at threshold= {voi_thresh} , VOI= {metrics[voi_thresh]['voi_sum']}, VOI_split= {metrics[voi_thresh]['voi_split']} , VOI_merge= {metrics[voi_thresh]['voi_merge']}"
        )
...
def evaluate(
    threshold, fragments, gt, fragments_file, crop_name, edges_collection, metrics
):
    segment_ids = get_segmentation(
        fragments, fragments_file, edges_collection, threshold
    )

    results = evaluate_threshold(
        edges_collection, crop_name, segment_ids, gt, threshold
    )
...
def get_segmentation(fragments, fragments_file, edges_collection, threshold):
    ...
    fragment_segment_lut_dir = os.path.join(fragments_file, "luts", "fragment_segment")
...
def evaluate_threshold(edges_collection, crop_name, test, truth, threshold):
